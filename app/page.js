...
function scoreSurf(h, spot, tideCtx) {
  ...
  // short, the wave face is textured and hard to read. This is the #1 reason
  // a "decent groundswell day" still surfs like junk.
...
function classifyConditions(userLevel, h, spot) {
  ...
  // Wind classification — learners need much tighter thresholds because
  // they haven't built the paddling + wave-reading skills to handle chop.
  // What's "bumpy but workable" for an intermediate is already "you're out
  // of your depth" for a first-timer.
  const isLearner = userLevel === "first_timer" || userLevel === "beginner";
  const blownNonOffshore = isLearner ? 18 : 25;  // non-offshore ≥ this km/h = blown
  const galeOffshore = isLearner ? 45 : 55;      // offshore ≥ this km/h = blown (strong offshore is hard to paddle against too)
  ...
  // Current velocity (m/s) — strong currents = rip-risk for beginners.
  // Open-Meteo returns m/s; 0.28 m/s ≈ 1 km/h, 0.56 m/s ≈ 2 km/h.
...
function getPersonalVerdict(userLevel, h, spot) {
  ...
  // against a rip. "dangerous" = hard no; "strong" = capped to OK at best.
  ...
  // If there's a strong current, cap the best verdict to "ok" so a learner
  // is never told "GO" on a rippy day.
