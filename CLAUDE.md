...
- `prodScoring.js` — toute la logique métier surf. **Moteur UNIQUE, 100% CONTINU** (sprint 2026-07 : plus aucun palier dur — toutes les tables passent par `lerpTable`, tous les caps sont des rampes ; tests de continuité assert saut < 3 pts par pas fin).
  ...
  - **Point d'échantillonnage du VENT** (`windSamplePoint`, `resolveSeaWind`) : ⚠️ le vent NE se lit PLUS aux coordonnées du spot — ne pas revenir en arrière. Interroger les coordonnées du spot ne rend PAS "le vent au spot" : l'API snappe sur la cellule d'un modèle global (0.1° à 0.25° = 11 à 28 km) dont le CENTRE est dans les terres. Le choix réel n'est donc pas "plage ou large", c'est "toits de banlieue ou eau". Le vent 10 m est un DIAGNOSTIC extrapolé par loi log avec la rugosité de la cellule — banlieue z0 ≈ 0.3 m contre mer z0 ≈ 0.0002 m, ×1.3 rien que là, bien pire sur un offshore. Terrain Trigg 14/09 : app 10 km/h SE, 20+ ressenti SUR LA PLAGE — le facteur 2 était côté terre, sur un offshore, le cas le plus défavorable. Comme `h.windSpeedKn` alimente scoreV2 ET classifyConditions ET les tips, la sous-lecture rendait TOUTE la chaîne optimiste ("conditions plus compliquées que ce qu'annonce l'app").
    **Aller JUSTE assez loin, pas plus.** Contrainte contradictoire : trop près on ne sort pas de la cellule terrestre et on ne corrige rien ; trop loin on sur-lit les vents OFFSHORE, qui continuent d'accélérer au large (la couche limite interne met ~400 m de fetch à s'ajuster à 10 m de haut), donc on dégraderait justement les bonnes conditions. On ne devine donc AUCUNE distance : l'API renvoie le CENTRE de la cellule réellement utilisée (`latitude`/`longitude` de la réponse), on sonde `WIND_PROBE_KM = [4, 8, 14]` sur le cap du large et on retient LE PLUS PROCHE dont le centre de cellule est vraiment au large de celui du spot (`seawardGainKm > 0.5`). Ça s'adapte tout seul à la résolution du modèle au lieu de la supposer. Distance retenue mise en cache par coordonnées (`surf-wind-offset-<lat>,<lng>`) → une seule sonde par spot et par appareil, ensuite la requête retombe à 2 coordonnées. `WIND_OFFSET_KM = 8` n'est qu'un repli (premier chargement du passé, override `windLat`/`windLng`).
    Mécanique : la requête forecast envoie `latitude=spot,c1,c2…` → l'API renvoie un TABLEAU. `resolveSeaWind` prend `wind_speed_10m`/`wind_direction_10m`/`wind_gusts_10m` sur la cellule retenue et garde `temperature_2m`/`precipitation_probability`/`daily` sur la cellule du SPOT (c'est l'air de la plage que l'utilisateur ressent, pas celui du large). Best-effort strict : réponse mono-point, série absente/vide/désalignée, candidat retombé à terre (`elevation > 2 m`), ou aucun candidat qui sorte de la cellule → réponse du spot intégrale = comportement d'avant. Le PASSÉ n'envoie jamais plus de 2 coordonnées (quota : un 429 sur le tier gratuit = mock, le pire des UX). Verrouillé par `tests/wind-sample.test.mjs`.
    ⚠️ Ce qui reste NON calibré, c'est le vent lui-même contre un anémomètre. Vérité terrain Perth : stations BoM Ocean Reef / Swanbourne. Même règle que `swellAttenuation` — ne pas bouger au doigt mouillé.
  ...
  - ⚠️ `swellAttenuation` n'est TOUJOURS PAS calibrée (valeurs Perth 0.55-0.60 posées au jugé). Elle s'applique désormais à une valeur saine, mais le bon coefficient demande les bouées waverider DoT WA (Cottesloe / Rottnest, données horaires gratuites via AODN). Ne pas la retoucher au doigt mouillé.
...
- `HourlyList.jsx` — Cards mode + List mode. Props `isToday`/`isPastDay` conditionnent le dimming "past". PAS de `key={level}` (reset le viewMode). La cellule WIND du panneau cards affiche la RAFALE (`G24`, `.hly-cp-cell-gust`) dès qu'elle dépasse la moyenne de 8 km/h : `windGustKn` était fetché depuis toujours mais ne vivait que dans StickyInfoBar, cachée par CSS dans les 2 modes → invisible en prod. Or c'est la rafale qu'on ressent debout sur la plage, et la FAQ promettait déjà cette note.
