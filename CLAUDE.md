...
- `prodScoring.js` — toute la logique métier surf. **Moteur UNIQUE, 100% CONTINU** (sprint 2026-07 : plus aucun palier dur — toutes les tables passent par `lerpTable`, tous les caps sont des rampes ; tests de continuité assert saut < 3 pts par pas fin).
  ...
  - **Point d'échantillonnage du VENT** (`windSamplePoint`, `mergeSeaWind`) : ⚠️ le vent NE se lit PLUS aux coordonnées du spot — ne pas revenir en arrière. Même artefact de bord, sur la grille ATMOSPHÉRIQUE cette fois (0.1° à 0.25° = 11 à 28 km) : un spot sur le trait de côte snappe sur une cellule dont le centre est dans les terres. Le vent 10 m est un DIAGNOSTIC extrapolé par loi log avec la rugosité de la cellule — banlieue z0 ≈ 0.3 m contre mer z0 ≈ 0.0002 m, soit ×1.3 rien que là, et ×1.5 à ×2 pour un vent OFFSHORE dont la couche limite interne se reconstruit sur les premiers km d'eau. Terrain Trigg 14/09 : app 10 km/h SE, 20+ mesuré sur la plage. Comme `h.windSpeedKn` alimente scoreV2 ET classifyConditions ET les tips, la sous-lecture rendait TOUTE la chaîne optimiste ("conditions plus compliquées que ce qu'annonce l'app"). Le vent part donc d'un point décalé de **12 km** (`WIND_OFFSET_KM`, plus loin que le marin : la grille atmo est plus grossière) sur le même cap (`idealSwellDir` curé, ou cap sondé par `probeOffshoreBearing` pour les spots persos). Override via `windLat`/`windLng`.
    Mécanique : les deux requêtes forecast envoient **2 coordonnées** (`latitude=spot,mer`) → l'API renvoie un TABLEAU. `mergeSeaWind` prend `wind_speed_10m`/`wind_direction_10m`/`wind_gusts_10m` sur la cellule MER et garde `temperature_2m`/`precipitation_probability`/`daily` sur la cellule du SPOT (c'est l'air de la plage que l'utilisateur ressent, pas celui du large). Best-effort strict : réponse mono-point, série absente/vide/désalignée, ou point mer retombé à terre (`elevation > 2 m` — baie fermée, île en face) → réponse du spot intégrale = comportement d'avant. Verrouillé par `tests/wind-sample.test.mjs`.
    ⚠️ `WIND_OFFSET_KM` n'est PAS calibré contre un anémomètre. Vérité terrain pour Perth : stations BoM Ocean Reef / Swanbourne. Même règle que `swellAttenuation` — ne pas bouger au doigt mouillé.
...
- Tests vitest : `tests/scoring.test.mjs` (52 cas) — invariants verdict/ceilings/grille/NaN + continuité + dominante + sécurité ; `tests/wind-sample.test.mjs` (19 cas) — géométrie du point vent + garde-fous du recollage mer/spot
