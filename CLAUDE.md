...
- `prodScoring.js` — toute la logique métier surf. **Moteur UNIQUE, 100% CONTINU** (sprint 2026-07 : plus aucun palier dur — toutes les tables passent par `lerpTable`, tous les caps sont des rampes ; tests de continuité assert saut < 3 pts par pas fin).
  ...
  - **Sonde du cap du large (`probeOffshoreBearing`) — TOUS les spots, curés compris.** Elle sonde une couronne de 8 caps à 5 km en UNE requête (l'API accepte `latitude=a,b,c`) et mesure la houle moyenne de chacun ; les cellules à terre sont masquées/quasi vides et éliminées. Cache par coordonnées (`surf-marine-bearing-v2-<lat>,<lng>`) → une sonde par spot et par appareil. Best-effort strict : échec réseau, HTTP non-ok ou sonde non concluante → `null` → comportement d'avant.
    ...
    - ⚠️ **`PROBE_MIN_GAIN` n'est PAS un seuil de confort.** Il ne sert qu'à éviter de basculer entre deux caps qui tombent dans la MÊME cellule (même série, même moyenne). Une version a posé **1.15 « par prudence »** : sur les 111 spots ça annulait la correction sur 57 d'entre eux, donc ça **désactivait la feature en silence** au lieu de la sécuriser. Le vrai garde-fou est la contrainte ±90°, pas ce chiffre. Il est à 1.02.
...
- Tests vitest : `tests/scoring.test.mjs` (107 cas, dont 11 cas terrain) — invariants verdict/ceilings/grille/NaN + continuité + dominante + sécurité ; `tests/wind-sample.test.mjs` (23 cas) — géométrie du point vent, sélection de la cellule la plus proche qui sort vraiment de la terre, garde-fous du recollage mer/spot
