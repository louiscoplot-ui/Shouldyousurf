...
- Analytics : PostHog US Cloud + Session Replay (`NEXT_PUBLIC_POSTHOG_KEY`) + GA4 (gtag dans layout.js)
  ...
  - ✅ **Vercel Web Analytics : ACTIVÉ** (`@vercel/analytics`, monté via `app/VercelAnalytics.jsx`). C'est le **SEUL des trois** que Claude peut lire directement depuis une session, via le MCP Vercel (`get_web_analytics` : visiteurs, pages vues, **pays**, routes, referrers, appareils). Ni PostHog ni GA4 n'ont de MCP disponible ici — leurs chiffres se consultent à la main.
  - ✅ **UN APPAREIL = UNE PERSONNE** (`deviceId()` / `identifyDevice()` dans `app/lib/analytics.js`, appelé une fois au montage de `MainScreen`). ⚠️ **C'ÉTAIT UN VRAI TROU** : `posthog.init` tourne avec `person_profiles: 'identified_only'` et `identify()` n'était appelé **NULLE PART** dans l'app. Cette combinaison ne crée **aucun profil de personne** — le compteur « combien de gens distincts » de PostHog était **vide par construction**, alors que les events partaient normalement. Corrigé par un UUID aléatoire généré une fois et gardé en localStorage (`ss-device-id`) : aucune donnée personnelle, aucune empreinte de navigateur, juste un identifiant first-party. Un appareil qui revient garde le même → il compte pour UNE personne quel que soit le nombre d'ouvertures. Honore `?noanalytics=1`.
    - ⚠️ Vider le stockage, ou utiliser un autre navigateur sur le même téléphone, crée un nouvel ID donc une personne de plus. **Inhérent à toute mesure sans compte utilisateur** — seul un login ferait mieux, et l'app n'en a pas.
  - **Quelle métrique lire pour « combien de personnes »** :
    - **GA4 → « Users »** : déjà unique par appareil via le cookie `_ga` persistant. C'est la réponse la plus proche de la question.
    - **PostHog → « Persons »** : unique par appareil et persistant, **à partir du fix ci-dessus seulement**.
    - **Vercel → « Visitors »** : ⚠️ cookieless, donc unique **PAR JOUR**. Quelqu'un qui revient 5 jours compte **5 visiteurs** sur la période. C'est une limite du produit, pas un bug à corriger — ne pas lire ce chiffre comme un nombre de personnes.
    - ⚠️ **`VercelAnalytics.jsx` est un wrapper CLIENT, et ce n'est pas du zèle** : `layout.js` est un Server Component (il exporte `metadata`), donc il ne peut pas passer une FONCTION en prop. Or `beforeSend` est une fonction, et sans elle Vercel compterait Louis parmi ses propres visiteurs. **Les trois outils doivent honorer le MÊME interrupteur `?noanalytics=1`**, sinon on obtient deux tableaux de bord qui se contredisent. Ne pas « simplifier » en montant `<Analytics />` directement dans `layout.js`.
    - Projet Vercel : `prj_ozh69HAci2vTm8RB4DWWVRAdlXih`, team `team_eM6hW9tPGhygYPRokZUPAKUK`, plan **Hobby** (quota d'événements mensuel, voir le dashboard).
    - ⚠️ Claude n'a PAS la permission de lister les variables d'env Vercel (403 sur `filter_project_envs`) : impossible de confirmer d'ici si `NEXT_PUBLIC_POSTHOG_KEY` est posée.
...
- ⚠️ CSS : `.wrap.hly-cardmode-active` et `.wrap:has(.hly--list-mode)` replient StickyInfoBar/`.C` et DrivingChips/`.drv` dans les DEUX modes. C'est VOULU : leur contenu a déménagé dans `.hly-cpanel`. Décision produit toujours en attente pour ces deux-là (réintégrer ou supprimer).
  - ✅ **`.best` (BestWindow) est RALLUMÉ dans les deux modes.** Il était replié par **dommage collatéral** : balayé avec `.C` et `.drv` alors qu'il n'a **aucun remplaçant** dans `.hly-cpanel`. Résultat, l'app ne disait plus **à quelle heure y aller aujourd'hui** — la question qu'elle est censée trancher. Si tu veux le rereplier c'est une ligne, mais fais-le en connaissance de cause.
...
- Strings UI hardcodées EN restantes (chips, LevelMatrix reasons, ScoreSheet notes, session notes). ✅ **Les 10 libellés de `HourlyList` + les 3 de `BestWindow` sont traduits dans les 12 langues** (clés `lbl_*`, `best_*`). ⚠️ Ces traductions n'ont **pas été relues par des natifs** : le repli anglais de `getT` protège, mais si un libellé choque, c'est une ligne dans `app/i18n.js`. Les clés safety (danger_banner, footer_disclaimer, mock_banner) SONT traduites partout.
- StickyInfoBar / DrivingChips cachés par CSS dans les 2 modes (décision produit en attente). BestWindow, lui, est rallumé.
- **surfer.mp4 : 5.3 MB, 80 % de `/public`.** ✅ Il n'est plus en `preload="auto"` avec un `src` en dur : le `src` est posé par un script inline qui **saute le chargement** si `navigator.connection` signale `saveData` ou un réseau `slow-2g`/`2g`/`3g`. ⚠️ **Safari/iOS n'expose pas `navigator.connection`** → sur iPhone le comportement est inchangé, la vidéo se charge. Le vrai fix reste le ré-encodage (~1.5 MB), impossible ici : **pas de ffmpeg dans l'env de session (vérifié, pas déduit)**. Commande à lancer en local : `ffmpeg -i surfer.mp4 -vf scale=-2:720 -c:v libx264 -crf 30 -preset slow -an -movflags +faststart surfer.mp4`.
