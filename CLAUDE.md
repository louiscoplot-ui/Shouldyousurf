...
- `prodScoring.js` — toute la logique métier surf. **Moteur UNIQUE, 100% CONTINU** (sprint 2026-07 : plus aucun palier dur — toutes les tables passent par `lerpTable`, tous les caps sont des rampes ; tests de continuité assert saut < 3 pts par pas fin).
  ...
  - **Vent RESSENTI** (`feltWindKmh`) : ⚠️ pour les learners le moteur NE classe PLUS sur la moyenne seule. Relevé Open-Meteo brut, Trigg 14/09 18h15 : **moyenne 10.3 km/h, rafales 22.3, facteur 2.17**. L'app affichait "10 km/h · clean", sur place ça soufflait visiblement plus. Un humain debout sur une plage, et la surface de l'eau, encaissent les RAFALES, pas la moyenne sur 10 min. `feltWindKmh = moyenne + (rafale − moyenne) × GUST_WEIGHT` (0.5), lu par `classifyConditions` ET par le plafond du verdict. Rafale absente ou sous la moyenne → moyenne, on ne fabrique rien. `gustMult` de scoreV2 note désormais aussi le **RATIO** rafale/moyenne (min des deux tables) : +12 km/h sur une moyenne de 40 c'est du bruit, sur une moyenne de 10 c'est un vent qui double — l'ancienne table ne coûtait que 1.4 % sur ce relevé. ⚠️ `GUST_WEIGHT` n'est PAS calibré (0.5 = point neutre). Même règle que `swellAttenuation`.
